...
def about_ms(par=None):
    def create_about_ms():
        ...
        info_string_2 = Label(m.info_window, text='Версия:   14.2.3')
...
def home():
    def on_M(par=None):
        m.monkey_control.append('M')
        return par

    def on_O(par=None):
        if m.monkey_control == ['M']:
            m.monkey_control.append('O')
        else:
            m.monkey_control.clear()
        return par

    def on_N(par=None):
        if m.monkey_control == ['M', 'O']:
            m.monkey_control.append('N')
        else:
            m.monkey_control.clear()
        return par

    def on_K(par=None):
        if m.monkey_control == ['M', 'O', 'N']:
            m.monkey_control.append('K')
        else:
            m.monkey_control.clear()
        return par

    def on_E(par=None):
        if m.monkey_control == ['M', 'O', 'N', 'K']:
            m.monkey_control.append('E')
        else:
            m.monkey_control.clear()
        return par

    def on_Y(par=None):
        if m.monkey_control == ['M', 'O', 'N', 'K', 'E']:
            monk_window = Toplevel()
            monk_window.title('monkey...')
            center_window(monk_window, 240, 178)
            monk_window.resizable(False, False)
            monk_gif = Gif(monk_window, path='monkey.gif')
            monk_gif.pack()
            m.monkey_control.clear()
        else:
            m.monkey_control.clear()
    ...
    root.bind('<KeyPress-m>', on_M)
    root.bind('<KeyPress-o>', on_O)
    root.bind('<KeyPress-n>', on_N)
    root.bind('<KeyPress-k>', on_K)
    root.bind('<KeyPress-e>', on_E)
    root.bind('<KeyPress-y>', on_Y)
