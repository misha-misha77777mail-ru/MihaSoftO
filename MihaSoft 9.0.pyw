...
def fallout1():
    ...
    def function():
        global m
        if not txt1.get().isdigit() or not txt2.get().isdigit() or not txt3.get().isdigit() or not txt4.get().isdigit() or not txt5.get().isdigit() or not txt6.get().isdigit():
    ...
    but5 = Button(window, text="Очистить", width=13, height=1, bg="#ebd38a", fg="black", command=functionu)
...
def fallout5():
    ...
    ToolTip(btn4, 'Удалить существующий отчёт...')
...
def fallout10():
    global k, kk, flag, speed_push, speed_snar
    def returniq():
        window2.destroy()
        window.state('normal')
    ...
    def zalp():
        global flag, k, kk, speed_snar

        def odin():
            f = open('C:/MihaSoft Files/sl.miha', 'r')
            l = f.read()
            f.close()
            l = int(l)
            z = l
            z = z + 5
            snaryad.place(x=z)
            f = open('C:/MihaSoft Files/sl.miha', 'w')
            f.write(str(z))
            f.close()
    ...
    def vverh_abs():
        global flag, speed_push
        flag = True
        f = open('C:/MihaSoft Files/sl1.miha', 'w')
        d = str(pushka.winfo_y())
        f.write(d)
        f.close()

        def odin():
            if pushka.winfo_y() > 130:
                f = open('C:/MihaSoft Files/sl1.miha', 'r')
                l = f.read()
                f.close()
                l = int(l)
                z = l
    ...
    window2.title('Shmalyalka 1.1')

    lbl1 = Label(window2, text="Shmalyalka 1.1", font=("Arial Bold", 16), fg="red")
    lbl1.place(x=40, y=20)
...
def fallout11():
    ...
    if os.path.exists('C:/MihaSoft Files/P Files') == False:
        os.mkdir('C:/MihaSoft Files/P Files')

    window3 = Toplevel()
    window3.geometry('550x300+50+50')
    window3.title('Paint 1.0')

    lbl1 = Label(window3, text="Paint 1.0", font=("Arial Bold", 16), fg="red")
