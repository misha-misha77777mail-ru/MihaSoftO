...
def fallout1():
    ...
    def functiony():
        def sav():
            if os.path.exists('C:/YAO Files') == False:
                os.mkdir('C:/YAO Files')
            a = str(tex.get())
            text_file = open("C:/YAO Files/" + a, "w")
            b = str(txt1.get())
            c = str(txt2.get())
            d = str(txt3.get())
    ...
    def functionz():
        def openz():
            a = os.listdir('C:/YAO Files')
            if a != []:
                b = str(zol1.get())
                fa = open("C:/YAO Files/" + b, "r")
                con = fa.read()
                fa.close()
                print(con)
                x = list(str(con))
        ...
        btt1 = Button(wind1, text="Подставить", font=("Arial Bold", 10), fg="black", bg='#7bd491', width=14, command=openz)
...
def fallout3():
    ...
    def f1():
        ...
        tox.insert(5.0, '  2 строка - объявление типа: using input; (вводить числа с клавиатуры) или using const; (конкретные числа)\n')
        tox.insert(6.0, '  _____________\n')
        tox.insert(7.0, '  3 строка - если using input;: method=x;, где x это с (сложение), в (вычитание), у (умножение), д (деление), ц (целочисленное деление), д (деление с остатком)\n')
        ...
        tox.insert(13.0, '  5 строка - если using const;: method=x;, где x это с (сложение), в (вычитание), у (умножение), д (деление), ц (целочисленное деление), д (деление с остатком)\n')
...
def fallout5():
    ...
    def iniz():
        ...
        if ((flag1 == 1 and flag2 == 1 and flag3 == 1) or (flag4 == 1 and flag5 == 1 and flag6 == 1) or (
            ...
            okno()
            plr_res = plr2
        elif (flag1 != 3 and flag2 != 3 and flag3 != 3 and flag4 != 3 and flag5 != 3 and flag6 != 3 and flag7 != 3 and flag8 != 3 and flag9 != 3):
            zin()
            a = 4
            q = False
            z = True
...
if os.path.exists('C:/MihaSoft Files') == False:
    ...
    lbli = Label(window, text='Вас приветствют разработчики MihaSoft! Для корректного продолжения работы\n программы необходимо разрешение на создание системных папок на диске C:')
    lbli.place(x=5, y=5)
