...
def YourAge():
    ...
    def yao_to_home():
        ...
        center_window(root, 718, m.abs_height)
...
Пароль: {am_input_3.get()}''')
                    ...
                    messagebox.showwarning('Ошибка!', 'Файл по умолчанию не найден!')
            else:
                default_file()
        else:
            messagebox.showwarning('Ошибка!', 'Заполните поля перед сохранением!')
        ...
        def bac_answer():
            ...
            if bac.flag:
                if len(bac_input.get()) != 3 or bac_input.get()[0] == '0':
                    messagebox.showwarning(parent=m.bac_window, title='Ошибка!',
...
def makeQR():
    def create_makeQR():
        ...
        # qr_read_button = Button(qr_window, text='СЧИТАТЬ', bg='#e6c687', width=10,
        #                         font=('Times New Roman', 12), command=qr_read)
        # qr_read_button.place(x=142, y=130)
        ...
        # qr_read_button = Button(qr_window, text='СОХРАНИТЬ ТЕКСТ', bg='#87d6e6', width=20,
        #                         font=('Times New Roman', 12), command=qr_make)
        # qr_read_button.place(x=97, y=175)
    try:
        m.qr_window.resizable(False, False)
...
def about_ms():
    def create_about_ms():
        ...
        info_string_2 = Label(m.info_window, text='Версия:   14.2')
...
def home():
    ...
    ms_mso_button.place(x=380, y=200)
...
if __name__ == '__main__':
    ...
    if os.path.exists('C:/MihaSoft Files') and os.path.exists('C:/MihaSoft Files/AnimationFlagFile.miha'):
        ...
        hello_title = Label(root, text='MihaSoft\n14.2', bg='red', fg='white', font=('Arial Bold', 60), height=100)
        hello_title.pack(expand=1)
        root.after(3000, hello_end)
